...
==最后==，三个新的 MLP $\mathcal{F}_i^{(2)}$ 用于基于全局特征和第一阶段的结果预测更精确的结果：$m_i^{(2)} = \mathcal{F}_i^{(2)}(f_{\rm in}, f_g, m_1^{(1)}, m_2^{(1)}, m_3^{(1)}),\ i=1,2,3$。在第二阶段优化第一阶段结果的同时，初始掩码 $m_i^{(1)}$ 也有助于第二阶段将全局特征提取聚焦于需要分割的部件，使特征抽取更加高效，并提升分割结果的准确率。 
