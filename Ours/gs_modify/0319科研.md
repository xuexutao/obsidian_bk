...
⏺ 可以改，而且我已经把“像素级 mask gating”真正下沉到 CUDA rasterizer 里了：现在在 renderCUDA 每个线程对应的像素上，会先判断该像素是否在当前视角的 mask 内；不在 mask    
  ...
  rasterization_masked/diff_gaussian_rasterization/**init**.py:83                                                                                                       
...
进入代码目录：/mnt/bn/aidp-data-3d-lf1/xxt/merlin/gs/workspace/GS_dev。使用conda环境gaussian。我上一次让你做的优化已经写在/mnt/bn/aidp-data-3d-lf1/xxt/merlin/gs/w │

│   orkspace/GS_dev/0321_0028_addmask.md这个里面了，但是我对你现在的实现还有点不满意。在/mnt/bn/aidp-data-3d-lf1/xxt/merlin/gs/workspace/GS_dev/submodules/diff-gaussi │

│   an-rasterization_masked/cuda_rasterizer/forward.cu 里面的renderCUDA                                                                                                │

│   还是一个方框一个方框的去做渲染，能否修改这个逻辑，而是把我们mask出来的部分当作全部，直接一个group渲染512*512个mask的像素，而不是现在这种先判断像素在不在mask中，再 │

│   决定渲染不渲染，我主要是想如果做mask里的像素他速度会更快一些。而且你也要实现在/mnt/bn/aidp-data-3d-lf1/xxt/merlin/gs/workspace/GS_dev/train.py                     │

│   训练代码里，构建的loss，要拓展我们mask出来的loss，也就是总体的loss再加上我们单个物体的loss加权，最终更加优化我们生成的全场景，以及全场景里的单个GS物体，你完整的实 │

│   现，从数据准备到训练，你测试通。主要是renderCUDA和preprocessCUDA                                                                                                   │

│   以及他们相对应的backward.cu里的反向传播。你可以详细整理整理，想清楚再实现，我是准备以此作为创新点发论文呢，你看看怎么来比较好。你可以把你做的修改完成的事情写到/mn │

│   t/bn/aidp-data-3d-lf1/xxt/merlin/gs/workspace/GS_dev/0321_night.md这里面。多写点代码，我不着急，有一个晚上时间呢。 

##### 如何运行

2) 数据准备：生成多视角一致 SAM masks（如果你还没生成）                                
  以 garden 为例（images_8）：              
  ...
  3) 训练：全场景 + mask 辅助 + 单物体加权（你要的论文版目标）                           

  python train.py \                         
    -s data/gs_data/garden \                
    -m output/0321_run_garden \             
    --images images_8 \                     
    --disable_viewer \                      
    --iterations 30000 \                    
    ...
    --mask_single_object_weight 0.2 \       
    --mask_single_object_strategy largest

4) 只训练某个物体（纯 mask 内像素，验证“只渲染 mask 像素”的加速路径）                  
  例如只训 obj_0002：                       

  python train.py \                         
    -s data/gs_data/bicycle \               
    -m output/0321_run_obj0002 \            
    --disable_viewer \                      
    --iterations 7000 \                     
    --mask_only \                           
    --mask_object_id 2 \                    
  ...
  • CUDA version mismatch (12.x vs 11.7)：按上面第 1 步固定 CUDA_HOME=/usr/local/cuda-11.7 再重装。                                                                               
  • IndexError: arch_list[-1]（通常是没法自动推断 GPU 架构）：必须 export TORCH_CUDA_ARCH_LIST="8.0"（按你的卡型调整）。

### 复现LangSplat

![](assets/0319科研/file-20260321164729581.png)
