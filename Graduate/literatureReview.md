...
《Instance-aware 3D Gaussian Splatting》在 SemGaussian 的基础上，该工作更进一步，实现了实例级别的语义理解。它为每个3D高斯分配一个实例ID，使得模型不仅能识别“这是椅子”，还能区分“这是哪一把椅子”，这对于需要与特定物体交互的场景（如机器人抓取）至关重要。然而，其实例分配的精度依赖于底层几何的准确性，且实例信息同样未用于指导高斯的几何优化。
