...
InstantNGP（即时神经图形基元）的核心创新是设计多分辨率哈希编码（Multiresolution Hash Encoding），大幅降低神经辐射场（NeRF）的训练与推理开销，将原本数小时的训练时间压缩至分钟级，同时保持高保真重建质量，解决传统 NeRF 算力依赖强、实时性差的核心痛点。其高效编码方案为轻量化融合模型设计提供关键技术支撑，可用于优化重建与理解模块的推理速度，突破了隐式重建的效率瓶颈，与 3DGS 形成高效隐式表示的两大核心技术路线。
...
进入 ECCV 2024，MVSplat（Oral）成为多视图 feed-forward 3DGS 的代表性突破。该方法通过 plane sweeping 构建多视图 cost volume，以此编码跨视角几何一致性，并联合预测深度与其他高斯属性。其核心洞见在于：将高斯中心定位问题重新表述为基于特征匹配识别表面点的任务，显著降低了学习难度。MVSplat 以轻量模型实现高效重建，但其性能仍受制于传统多视图立体视觉的固有缺陷，如遮挡与无纹理区域的匹配失败，这促使后续工作探索与单目深度先验的融合。
