...
对于给定多个视图图像作为输入，MVSplat首先使用Transformer提取多视图图像特征。然后，使用plane sweeping构建每个视图的cost volume。Transformer特征和cost volume被连接在一起，作为2D U-Net的输入，用于成本量细化和预测每个视图的深度图。每个视图的深度图未投影到3D，并使用简单的确定性联合操作作为3D高斯中心进行组合。不透明度、协方差和颜色高斯参数与深度图联合预测。最后，通过光栅化操作从预测的3D高斯图像中渲染出新的视图
