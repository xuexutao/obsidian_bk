...
DGCNN 则是从全局特征提取向局部结构建模演进的代表性工作，它是在 PointNet 的基础上进行了重要改进。通过引入动态图（Dynamic Graph）的概念，使模型重点理解点云中的局部几何结构。在每一层网络中，它会根据当前层的特征重新计算点与点之间的K近邻关系，构建一个动态的图结构，然后在这个图上应用图卷积操作来捕捉局部上下文信息。它使网络能够适应不同层次的语义信息，从而更好地建模点云的局部结构。DGCNN 解决了 PointNet 忽略局部结构的问题，在点云分割和分类任务上取得了显著优于 PointNet 的性能。然而，它的计算复杂度较高，且图的构建依赖于欧氏距离，对于非均匀采样的点云可能不够鲁棒。，为后续更复杂的图神经网络（GNN）在3D视觉中的应用铺平了道路。
