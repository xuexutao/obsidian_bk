...
在训练实例场时，NeRF-RCNN 会提取预训练 NeRF 的辐射场和密度场，并为 NeRF 中检测到的每个物体输出离散的 3D 掩码。Mask2Former 会对从 NeRF 渲染出的图像生成 2D 全景分割图，这些分割图在不同视图间的实例标签存在不一致性。通过将相同相机姿态下的 3D 掩码投影到图像空间，实现不同视图间同一实例的匹配，最终得到多视图一致的 2D 分割图；该一致分割图可用于训练实例场组件，并借助实例场表示生成连续的 3D 分割结果。此外，我们采用 CascadePSP 模型对 Instance-NeRF 的初步实例分割结果进行精炼，并利用精炼后的实例掩码反过来优化实例场。
