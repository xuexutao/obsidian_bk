...
在数据层面，SAM 3D 设计了模型在环（MITL）的人机协同标注流水线，解决 3D 标注成本高昂的核心痛点。该流水线将 “生成 3D 真值” 转化为 “选择 + 对齐 3D 候选”：普通标注者从模型生成的 8 个 3D 候选（来自检索、文本到 3D、图像到 3D 等方式）中筛选最优解，仅将模型无法处理的难例（如复杂几何、严重遮挡物体）交由专业 3D 艺术家标注，形成 Art-3DO 高质量数据集。最终生成 314 万无纹理网格、10 万带纹理网格及 700 万对偏好数据，规模远超现有真实 3D 重建数据集，同时发布 SA-3DAO 基准（1000 个由专业艺术家标注的真实图像 - 3D 配对数据），填补真实场景 3D 重建评估空白。

架构上，SAM 3D 采用双阶段重建设计：第一阶段为几何模型（1.2B 参数），基于混合 Transformer（MoT）架构，通过整流条件流匹配（CFM）预测粗粒度形状（64³ 体素）与布局参数（6D 旋转、3D 平移、3D 尺度），形状与布局共享特征空间且通过注意力掩码实现模态分离与信息交互，确保二者一致性；第二阶段为纹理与细化模型（600M 参数），基于稀疏 latent 流 Transformer，从粗粒度形状中提取有效体素，补充高频几何细节并生成像素对齐的纹理，支持通过 VAE 解码器输出网格或 3D 高斯溅射两种格式，其中改进的 Depth-VAE 仅对可见体素反向投影特征，提升纹理重建清晰度。输入编码采用 DINOv2 提取裁剪物体与全图特征，保留局部细节与全局上下文，还可可选融合 LiDAR 或单目深度点图，进一步优化布局预测。

训练方面，SAM 3D 借鉴大语言模型训练范式，采用 “合成预训练→半合成中训练→真实数据后训练” 三阶段流程：预训练阶段使用 Iso-3DO 数据集（270 万 Objaverse-XL 物体的 24 视角渲染图），学习形状与纹理的基础生成能力；中训练阶段基于 RP-3DO 数据集（6100 万半合成样本，将合成物体嵌入自然图像），提升模型对遮挡、场景上下文的鲁棒性，引入布局参数预测；后训练阶段通过监督微调（SFT）与直接偏好优化（DPO）对齐真实数据与人类审美 ——SFT 先用普通标注数据（MITL-3DO）缩小领域差距，再用艺术家数据（Art-3DO）提升细节质量，DPO 利用人类偏好数据（最优候选 vs 其他候选）减少 “漂浮伪影”“无底部网格” 等不良输出，最后通过模型蒸馏将几何模型推理步数从 25 步降至 4 步，实现亚秒级重建且不损失精度。

实验结果显示，SAM 3D 在真实世界场景中表现突出：在 SA-3DAO 基准上，F1@0.01 达 0.2344、vIoU 达 0.2311，Chamfer 距离仅 0.04，较 Trellis、Hunyuan3D-2.1 等方法平均提升 50% 以上；人类偏好测试中，真实世界物体重建的头对头胜率达 5:1，纹理重建胜率超 80%；场景布局预测上，Aria Digital Twin 基准的 3D IoU 达 0.4970、ADD-S@0.1 达 0.7673，大幅超越 MIDI 等联合生成模型，甚至优于 “3D 重建 + 专门姿态估计” 的流水线方法。此外，SAM 3D 泛化能力强劲，在遮挡严重、多物体、长尾类别物体上均保持稳定性能，零样本迁移到真实场景的成功率较 SplatSim 提升 11%。

不过，SAM 3D 仍存在局限：物体分辨率受限于 64³ 体素，复杂细节（如人体手部）重建精度不足；多物体重建缺乏物理交互推理（如碰撞、支撑关系）；纹理生成未充分利用物体姿态信息，对称物体可能出现纹理错位。未来方向包括提升输出分辨率、引入多物体物理约束、融合语言线索实现文本引导的 3D 编辑，进一步拓展其在机器人、AR/VR、游戏、影视等领域的应用场景。目前，SAM 3D 已开源模型权重、代码与在线演示，降低 3D 重建技术的应用门槛。
