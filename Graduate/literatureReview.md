...
DepthSplat 包含一个用于建模特征匹配信息的多视图分支，以及一个用于提取单目特征的单视图分支。该算法将各视图 Cost Volume 与单目特征进行拼接，再通过2D U-Net架构完成深度回归任务。在深度估计任务中，模型利用真实深度标注数据进行监督训练。对于高斯溅射任务，算法首先将所有深度图反投影至三维空间，以此作为高斯核的中心位置，同时通过一个额外的预测头生成其余高斯参数。新视角的图像则通过溅射渲染操作生成。用于新视角合成的完整模型采用光度渲染损失函数进行训练，该训练过程同时也可作为深度模型的无监督预训练阶段。
