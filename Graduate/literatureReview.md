...
Editing 3D Scenes with Gaussian Splatting 探索了3DGS的可编辑性。由于高斯椭球显式参数化的特点，3DGS天然比NeRF更容易进行编辑。这篇论文展示了如何对选定的高斯子集进行移动、缩放、删除或风格迁移等操作，并能实时看到编辑效果。基于此可以做到直观、实时的3D场景编辑，可以让用户方便地修改神经渲染场景。然而，编辑操作是几何和外观层面的，虽然有部分的场景语义信息的加入。
