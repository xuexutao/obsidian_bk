...
Semantic NeRF 将语义信息融入 NeRF，通过单一 MLP 同时建模 3D 位置相关的密度、视角相关的颜色及视角无关的语义逻辑，利用几何与语义的内在关联性提升标注效率。它在标准NeRF的基础上增加了一个分支，用于预测每个采样点的语义标签（如“墙壁”、“椅子”）。通过共享底层特征，实现了重建和语义预测的联合优化。然而，其局限性在于语义预测是像素级的，缺乏实例区分能力，且语义信息并未反向指导几何重建过程，两者仍是松耦合的。它标志着NeRF从“纯视觉”向“语义视觉”的初步迈进，但距离真正的“理解”还有很大差距。
