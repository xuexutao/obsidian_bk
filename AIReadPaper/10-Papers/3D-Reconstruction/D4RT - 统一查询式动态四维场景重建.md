...
|模块|内容|
|---|---|
|输入|单段视频 $V \in \mathbb{R}^{T \times H \times W \times 3}$。训练时使用 48 帧 clip，分辨率 256×256。|
...
2. **构造查询 token**
    1. 连续 2D 坐标 $(u,v)$ 先做 Fourier feature 编码。
    2. $t_{src}, t_{tgt}, t_{cam}$ 分别用离散时间嵌入表示。
    3. 额外拼入以 $(u,v)$ 为中心的 **9×9 局部 RGB patch** 嵌入。
3. **独立解码**
...
|辅助监督|2D 投影位置、法向、可见性、位移、置信度。附录给出权重：$\lambda_{3D}=1.0, \lambda_{2D}=0.1, \lambda_{vis}=0.1, \lambda_{disp}=0.1, \lambda_{normal}=0.5, \lambda_{conf}=0.2$。|
