...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/llava3d-1.png)

## 3D LLM

利用多视角几何恢复出来的场景，编码为3DFeature再输入到LLM中，相比于上一个只利用深度编码，对3d的理解会更好一点

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/3dllm-1.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointllm-1.png)

架构分为三部分：预训练的点云编码器 $f_{pe}$ ，投影器 $f_{proj}$ 和预训练的大语言模型（LLM）主干 $f_{llm}$

$f_{pe}$ 编码器将点云 $P \in \mathbb{R}^{n \times d}$ 作为输入【n = 8192 个点和 d = 6】输出点特征序列 $X = (x_1,x_2,\dots ,x_m) \in \mathbb{R}^{m\times c}$【m=513，c= 384】
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/point3dllm-1.png)

#### 3、3D-LLaVA CVPR 2025

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/3dllava-1.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/ulip-1.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/ulip-2.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclip-1.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclip-2.png)

给定CLIP 编码的 M 视角特征，沿通道维度将它们连结为 $\operatorname{Concat}(f_{1\sim M}) \in \mathbb{R}^{1\times MC}$

上述的concat后的特征，经过一个三层的多层感知机（MLP），得到所有视图汇总的全局特征。

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclip-3.png)

然后全局特征再经过MLP生成**视图自适应特征**，残差连接原始输入，得到最终的**Adapted Features**

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclip-4.png)

#### 2、基于多视图：PointCLIP V2 ICCV 2023

**PointCLIP有下面两个缺点：**

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclipv2-1.png)![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclipv2-2.png)

V2的改进：

将点云转为体素，然后经过模糊化、高斯过滤等操作，让渲染图不再是点图，而是渐变的**深度图**

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclipv2-3.png)![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclipv2-4.png)

V2的改进：

用GPT3生成丰富3D语意，会使得图文右更好的相似度

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointclipv2-5.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointnet-1.png)![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointnet-2.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointnet-3.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointnet-4.png)

改进版本，层次化点云特征提取：**PointNet++**

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointnetpp-1.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointmlp-1.png)

使用K=24的KNN选择中心点的邻居。密度不均匀

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointmlp-2.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/pointbert-1.png)

给定输入点 $p\in \mathbb{R}^{N\times3}$，首先通过最远点采样（FPS）从整体点云 p 中采样 $g$ 个中心点，使用k-近邻法为每个中心点选择 n 个最近邻点，生成局部patch $\{ p_i\}^g_{i=1}$，通过减去中心坐标，使得patch变为**无偏**，这些无偏patch就可以类比为NLP或者vit的 patch 了，然后使用**mini-pointnet**得到点云嵌入 --》 **点嵌入序列：** $\{f_i\}^g_{i=1}$
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/llamamesh-1.png)![](assets/三维多模态大语言模型架构%20-%20Survey/figures/llamamesh-2.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/llamamesh-3.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/llamamesh-4.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshllm-1.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshllm-2.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshcoder-1.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshcoder-2.png)

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshcoder-3.png)
...
![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshgpt-1.png)

#### 2、MeshAnything ICLR 2025

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/meshanything-1.png)

#### 3、基于多视角：InstantMesh github：4.1k

【图生3D Mesh】

![](assets/三维多模态大语言模型架构%20-%20Survey/figures/instantmesh-1.png)
