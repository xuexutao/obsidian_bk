...
**完整目标（ELBO 到底是什么）**：真正想最大化的是训练数据<span class='red'>的对数似<span>然 $\text{log}  p_θ(x)$，但它要对所有可能的 $z$ 积分，实际算不出来。ELBO（evidence lower bound，证据下界）是这个对数似然的一个下界，把下界顶上去就等于间接把似然往上推，"变分自编码器"里的"变分"指的就是这件事。写成损失就是两项加权相加：
